...
export class UserService {
  ...
  addUserToTask(userId: number, taskId: number): Observable<User> {
    return this.http.post<User>(
      `${environment.backendUrl}/users/add/${userId}/assignToTask/${taskId}`,
      null,
    );
  }
  ...
  deleteUserFromTask(userId: number, taskId: number): Observable<void> {
    return this.http.delete<void>(
      `${environment.backendUrl}/users/${userId}/removeFromTask/${taskId}`,
  ...
  AllUserInTask(taskId: number): Observable<User[]> {
    return this.http.get<User[]>(
      `${environment.backendUrl}/users/${taskId}/usersAssigned`,
    );
  }
  AllUserNotInTAsk(taskId: number): Observable<User[]> {
    return this.http.get<User[]>(
      `${environment.backendUrl}/users/${taskId}/usersNotAssigned`,
    );
  }

  assignUserToTask(userId: number, taskId: number): Observable<string> {
    return this.http.post<string>(
      `${environment.backendUrl}/users/${userId}/assignToTask/${taskId}`,
      null,
      { responseType: 'text' as 'json' },
    );
  }
  setLimitUser(userId: number, limit: number): Observable<String> {
    return this.http.put<String>(
      `${environment.backendUrl}/users/${userId}/setMaxTasksLimit`,
      limit,
      { responseType: 'text' as 'json' },
    );
  }
